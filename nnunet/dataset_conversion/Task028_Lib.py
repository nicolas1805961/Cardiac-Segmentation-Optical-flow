...
if __name__ == "__main__":
    ...
    out_folder = os.path.join('out', 'nnUNet_raw_data_base', 'nnUNet_raw_data', 'Task028_Lib')
    ...
    for p in patient_dirs_train:
        current_dir = p
        info_path = os.path.join(current_dir, 'info.cfg')
        with open(info_path, 'r') as fd:
            original_path_list = fd.read().splitlines()
        ...
        assert len(original_path_list) == len(data_files_train) == len(corresponding_seg_files)
        for d, s, original_path in zip(data_files_train, corresponding_seg_files, original_path_list):
            all_original_paths.append(original_path)
    ...
    assert len(all_train_files) == len(all_original_paths)
    ...
    json_dict['reference'] = "see Lib"
    json_dict['licence'] = "see Lib"
    ...
    json_dict['training'] = [{'image': "./imagesTr/%s.nii.gz" % i.split("/")[-1][:-12], "label": "./labelsTr/%s.nii.gz" % i.split("/")[-1][:-12], "original_path": "%s" % j} for (i, j) in
                             zip(all_train_files, all_original_paths)]
    ...
    save_pickle(splits, os.path.join('out', 'nnUNet_raw_data_base', 'nnUNet_raw_data', 'Task028_Lib', 'splits_final.pkl'))