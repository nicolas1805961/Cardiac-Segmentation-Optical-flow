...
from nnunet.lib.training_utils import build_model_1, build_final_flow_raft_model, build_2d_model, build_flow_model_video, build_discriminator, read_config_video, read_config, build_discriminator, build_discriminator, build_video_model
...
class FinalFlowRaft(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        timestr = datetime.now().strftime("%Y_%m_%H_%M_%S_%f")
    ...
    def setup_loss_data(self):
        ...
        if self.video_length > 2:
            loss_data['cumulated_flow_regularization'] = [self.regularization_weight_xy, float('nan')]
            ...
            loss_data['global_flow_regularization'] = [1.0, float('nan')]
    ...
    def initialize_network(self):
        ...
        self.network = build_final_flow_raft_model(self.config, image_size=self.crop_size, log_function=self.print_to_log_file, nb_channels=1)
    ...
    def compute_losses_recursive(self, unlabeled, out, target):
        ...
        if self.dataloader_modality == 'other':
            ...
            registered = self.motion_estimation(flow=out['flow'][-1], original=initial_target, mode='bilinear')
...
#
        ...
        if self.video_length > 2:
            ...
            for i in range(len(out['flow'])):
                registered = self.motion_estimation(flow=out['flow'][i], original=unlabeled[i + 1])
            ...
            self.loss_data['intermediate'][1] = intermediate_loss
    ...
    def compute_losses_recursive_adjacent(self, unlabeled, out, target, first_pred):

        if self.dataloader_modality == 'other':

            seg_loss_1 = self.segmentation_loss(out['seg'][-1], target[-1])
            self.loss_data['segmentation'][1] = seg_loss_1

            registered = self.motion_estimation(flow=out['flow'][0], original=out['seg'][0], mode='bilinear')
            seg_registered_loss_0 = self.segmentation_loss(registered, target[0])
            self.loss_data['seg_registered_0'][1] = seg_registered_loss_0

            seg_registered_loss_list = []
            for t in range(1, len(out['seg'])):
                registered = self.motion_estimation(flow=out['flow'][t], original=out['seg'][t], mode='bilinear')
                current_target = torch.softmax(out['seg'][t - 1], dim=1)

                seg_registered_loss = self.cross_entropy_loss(registered, current_target)
                seg_registered_loss_list.append(seg_registered_loss)

            self.loss_data['seg_registered'][1] = torch.stack(seg_registered_loss_list).mean()
        ...
        if self.video_length > 2:
            ...
            for i in range(len(out['flow'])):
                ...
                intermediate_register_list.append(registered)
            intermediate_registered = torch.stack(intermediate_register_list, dim=0)
            intermediate_loss = self.registration_loss(intermediate_registered, unlabeled[:-1])
            self.loss_data['intermediate'][1] = intermediate_loss

            cumulated_flow_regularization_loss = self.image_flow_loss_progressive(flow=out['flow'], iter_nb=self.iter_nb)
            self.loss_data['cumulated_flow_regularization'][1] = cumulated_flow_regularization_loss
