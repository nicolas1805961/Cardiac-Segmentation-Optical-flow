...
from nnunet.training.dataloading.dataset_loading import DataLoader2D, DataLoader2DMiddleUnlabeled, DataLoaderFlowTrainPredictionVal, unpack_dataset, DataLoaderVideoUnlabeled
...
from nnunet.lib.training_utils import build_2d_model, build_flow_model_labeled, build_discriminator, read_config_video, read_config, build_discriminator, build_discriminator, build_video_model
...
class nnMTLTrainerV2FlowLabeled(nnUNetTrainer):
    """
    ...
    def setup_loss_data(self):
        ...
        loss_data['semi_supervised_backward'] = [self.semi_supervised_backward_loss_weight, float('nan')]
    ...
    def initialize_network(self):
        ...
        self.network = build_flow_model_labeled(self.config, conv_layer_2d=conv_layer_2d, norm_2d=norm_2d, image_size=self.crop_size, log_function=self.print_to_log_file)
        if self.fine_tuning:
        ...
        label_input_data = torch.zeros(size=(self.config['batch_size'], 1, self.crop_size, self.crop_size))
        models['temporal_model'] = (self.network, [label_input_data, unlabeled_input_data])
    ...
    def get_gradient_images(self, label, unlabeled, indices, task):
        t, b, c, y, x = indices
        with torch.enable_grad():
            self.network.zero_grad()
            unlabeled.requires_grad = True
            output = self.network(label, unlabeled)
            if self.deep_supervision:
    ...
    def run_online_evaluation(self, out, unlabeled, target, seg_dice, target_mask):
        ...
        #gradient_image_unlabeled, gradient_x_unlabeled = self.get_gradient_images(target[0], unlabeled, indices, 'forward_flow')
        #gradient_image_seg, gradient_x_seg = self.get_gradient_images(target[0], unlabeled, indices_seg, 'seg')
    ...
    def compute_losses_all_to_all(self, target_mask, unlabeled, out, target):
        ...
        #    previous_idx = 0 if i == 0 else i - 1
        #    next_idx = -1 if i == len(interpolated) - 1 else i + 1
        #    previous_distance = self.mse_loss(interpolated[i], interpolated[previous_idx])
        #    next_distance = self.mse_loss(interpolated[i], interpolated[next_idx])
        #    interpolated_loss.append(previous_distance.view(1,))
        #    interpolated_loss.append(next_distance.view(1,))
        #self.loss_data['interpolation'][1] = torch.cat(interpolated_loss).mean()
...
#
        ...
        pred_registered_list_forward = []
        seg_registered_list_forward = []
        for t in range(len(out['forward_flow'])):
            current_flow = out['forward_flow'][t]
            pred_registered = self.motion_estimation(flow=current_flow, original=out['seg'][0], mode='bilinear')
            pred_registered_list_forward.append(pred_registered)
            initial_target_registered = self.motion_estimation(flow=current_flow, original=initial_target, mode='bilinear')
            seg_registered_list_forward.append(initial_target_registered)
        pred_registered_list_forward = torch.stack(pred_registered_list_forward, dim=0) # T, B, C, H, W
        seg_registered_list_forward = torch.stack(seg_registered_list_forward, dim=0) # T, B, C, H, W

        pred_registered_list_backward = []
        for t in range(len(out['backward_flow'])):
            current_flow = out['backward_flow'][t]
            pred_registered = self.motion_estimation(flow=current_flow, original=out['seg'][t].detach(), mode='bilinear')
            pred_registered_list_backward.append(pred_registered)
        pred_registered_list_backward = torch.stack(pred_registered_list_backward, dim=0) # T, B, C, H, W
        ...
        seg_registered_loss_target = self.segmentation_loss(seg_registered_list_forward[t_indices, b_indices], target[t_indices, b_indices])
        self.loss_data['seg_to_seg_forward'][1] = seg_registered_loss_target

        initial_target = target[t_indices, b_indices]
        initial_target = torch.nn.functional.one_hot(initial_target[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
        initial_target_registered = self.motion_estimation(flow=out['backward_flow'][t_indices, b_indices], original=initial_target, mode='bilinear')
        seg_registered_loss_target_backward = self.segmentation_loss(initial_target_registered, target[0, b_indices])
        self.loss_data['seg_to_seg_backward'][1] = seg_registered_loss_target_backward

        forward_zero_loss = self.mse_loss(out['forward_flow'][0], torch.zeros_like(out['forward_flow'][0]))
        backward_zero_loss = self.mse_loss(out['backward_flow'][0], torch.zeros_like(out['backward_flow'][0]))
        self.loss_data['first_flow'][1] = (forward_zero_loss + backward_zero_loss) / 2
        ...
        semi_supervised_forward_loss = self.segmentation_loss(seg_registered_list_forward.permute(1, 2, 3, 4, 0), semi_supervised_target.permute(1, 2, 3, 4, 0))
        self.loss_data['semi_supervised_forward'][1] = semi_supervised_forward_loss

        seg_registered_list_forward = torch.argmax(seg_registered_list_forward, dim=2)
        out['seg_registered_forward'] = seg_registered_list_forward

        new_target = target[0][None].repeat(len(pred_registered_list_backward), 1, 1, 1, 1)
        semi_supervised_backward_loss = self.segmentation_loss(pred_registered_list_backward.permute(1, 2, 3, 4, 0), new_target.permute(1, 2, 3, 4, 0))
        self.loss_data['semi_supervised_backward'][1] = semi_supervised_backward_loss
#
        #target_pred = torch.softmax(out['seg'], dim=2) # T, B, 4, H, W
        #pred_registered_loss_pred = self.cross_entropy_loss(pred_registered_list_forward.permute(1, 2, 3, 4, 0), target_pred.permute(1, 2, 3, 4, 0))
        #self.loss_data['pred_registered_pred'][1] = pred_registered_loss_pred
#
        #target_pred = torch.softmax(pred_registered_list_forward, dim=2) # T, B, 4, H, W
        #pred_registered_loss_pred_2 = self.cross_entropy_loss(out['seg'].permute(1, 2, 3, 4, 0), target_pred.permute(1, 2, 3, 4, 0))
        #self.loss_data['pred_registered_pred_2'][1] = pred_registered_loss_pred_2

        #for_visualization = torch.softmax(pred_registered_list_forward, dim=2)
...
#
    ...
    def run_iteration_train(self, data_generator):
        ...
        out = self.network(target[0], unlabeled)
...
#
    ...
    def run_iteration_val(self, data_generator):
        ...
        if self.log_images:
            ...
            out['seg'] = out['seg'][:16]
            out['seg_registered_forward'] = out['seg_registered_forward'][:16]
            out['registered_input_forward'] = out['registered_input_forward'][:16]
            out['forward_flow'] = out['forward_flow'][:16]
            unlabeled = unlabeled[:16]
            target = target[:16]
            target_mask = target_mask[:16]
...
#
        ...
        flow_target = self.processor.uncrop_no_registration(flow_target[:, None], padding_need=padding)[:, 0] # B, 1, H, W
        registered = self.processor.uncrop_no_registration(registered[:, None, None], padding_need=padding)[:, 0, 0] # B, H, W
