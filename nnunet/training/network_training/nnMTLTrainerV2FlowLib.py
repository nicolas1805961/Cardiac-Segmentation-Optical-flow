...
from nnunet.training.dataloading.dataset_loading import DataLoaderFlowTrainPredictionVal, DataLoaderFlowTrainLib, DataLoader2DMiddleUnlabeled, unpack_dataset, DataLoaderVideoUnlabeled
...
from nnunet.lib.training_utils import build_flow_model_lib, build_2d_model, build_flow_model_4, build_discriminator, read_config_video, read_config, build_discriminator, build_discriminator, build_video_model
...
from nnunet.training.dataloading.dataset_loading import DataLoaderFlowTrainPredictionVal, DataLoaderFlowTrain5, DataLoaderFlowTrainRecursiveVideo, DataLoaderFlowValidationOneStep
...
class nnMTLTrainerV2FlowLib(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.forward_flow_loss_weight = self.config['forward_flow_loss_weight']
        self.strain_loss_weight = self.config['strain_loss_weight']
    ...
    def setup_loss_data(self):
        ...
        loss_data['strain'] = [self.strain_loss_weight, float('nan')]
        loss_data['forward_flow_loss'] = [self.forward_flow_loss_weight, float('nan')]
    ...
    def initialize_network(self):
        ...
        #    self.network = build_flow_model_simple(self.config, image_size=self.crop_size, log_function=self.print_to_log_file)
        #elif self.model_name == 'recursive_video':
        #    self.network = build_flow_model_recursive_video(self.config, image_size=self.crop_size, log_function=self.print_to_log_file)
        #elif self.model_name == 'prediction':
        #    self.network = build_flow_model_prediction(self.config, image_size=self.crop_size, log_function=self.print_to_log_file)
        #else:
        #    self.network = build_flow_model_lib(self.config, image_size=self.crop_size, log_function=self.print_to_log_file)
    ...
    def compute_strain_mse(self, lv_strain_pred, rv_strain_pred, lv_strain_gt, rv_strain_gt):
        lv_mse = torch.abs(lv_strain_pred - lv_strain_gt) * 100
        rv_mse = torch.abs(rv_strain_pred - rv_strain_gt) * 100
        self.table['strain']['lv'].append(lv_mse.mean().cpu())
        self.table['strain']['rv'].append(rv_mse.mean().cpu())
    ...
    def start_online_evaluation(self, out,
        ...
        weights = out['weights'].permute(1, 0, 2, 3).contiguous() # B, T, H, W
        lv_strain_pred = out['lv_strain'].permute(1, 0) # B, T
        lv_strain_gt = lv_strain.permute(1, 0) # B, T
        ...
        with torch.no_grad():
            ...
            if self.log_images:
                for b in range(len(target)):
                    ...
                    self.vis.set_up_image_strain(seg_dice=seg_dice[b].mean(), gt=lv_strain_gt[b], pred=lv_strain_pred[b])
                    ...
                    self.vis.set_up_image_weights(seg_dice=seg_dice[b].mean(),
                                                        x=unlabeled[b, :, 0],
                                                        weights=weights[b])
    ...
    def log_strain(self, key):
        rv_strain = np.mean(self.table[key]['rv'])
        lv_strain = np.mean(self.table[key]['lv'])
        self.print_to_log_file("RV Strain", rv_strain)
        self.print_to_log_file("LV Strain", lv_strain)
        class_strain = {'RV': rv_strain, 'LV': lv_strain}
        self.writer.add_scalars('Epoch/Class ' + key.title(), class_strain, self.epoch)
    ...
    def finish_online_evaluation(self):
        ...
        if self.log_images:
            ...
            self.vis.log_strain_images(epoch=self.epoch)
    ...
    def run_online_evaluation(self, out, unlabeled, target, lv_strain, seg_dice):
        """
        ...
        indices = self.sample_indices(target)
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True,
        ...
        if self.inference_mode == 'sliding_window':
            ...
            ret = super().validate_flow_one_step_lib(processor=self.processor, log_function=self.print_to_log_file, step=1, do_mirroring=do_mirroring, use_sliding_window=use_sliding_window, step_size=step_size,
                            save_softmax=save_softmax, use_gaussian=use_gaussian,
                            overwrite=overwrite, validation_folder_name=validation_folder_name,
                            all_in_gpu=all_in_gpu, segmentation_export_kwargs=segmentation_export_kwargs,
                            run_postprocessing_on_folds=run_postprocessing_on_folds, debug=True, output_folder=output_folder, save_flow=True)
            ...
            ret = super().validate_flow_overlap_alt(processor=self.processor, log_function=self.print_to_log_file, do_mirroring=do_mirroring, use_sliding_window=use_sliding_window, step_size=step_size,
                                save_softmax=save_softmax, use_gaussian=use_gaussian,
                                overwrite=overwrite, validation_folder_name=validation_folder_name,
                                all_in_gpu=all_in_gpu, segmentation_export_kwargs=segmentation_export_kwargs,
                                run_postprocessing_on_folds=run_postprocessing_on_folds, debug=True, output_folder=output_folder, save_flow=True)
    ...
    def compute_losses_recursive(self, unlabeled, out, target, lv_strain, rv_strain):
        out['registered_input_backward'] = []
        out['registered_input_forward'] = []
        out['seg_registered_forward'] = []

        seg_loss_l = self.segmentation_loss(out['seg'].permute(1, 2, 3, 4, 0), target.permute(1, 2, 3, 4, 0))
        self.loss_data['segmentation'][1] = seg_loss_l

        lv_strain_loss = self.mse_loss(out['lv_strain'], lv_strain)
        rv_strain_loss = self.mse_loss(out['rv_strain'], rv_strain)
        self.loss_data['strain'][1] = (lv_strain_loss + rv_strain_loss) / 2

        seg_registered_list_forward = []
        for t in range(1, len(out['forward_flow'])):
            current_flow_forward = out['forward_flow'][t]
            current_initial_target = target[t - 1]
            current_initial_target = torch.nn.functional.one_hot(current_initial_target[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
            current_local_registered_forward = self.motion_estimation(flow=current_flow_forward, original=current_initial_target, mode='bilinear')
            seg_registered_list_forward.append(current_local_registered_forward)
        seg_registered_list_forward = torch.stack(seg_registered_list_forward, dim=0) # T, B, C, H, W

        for_visualization = torch.argmax(seg_registered_list_forward, dim=2)
        out['seg_registered_forward'] = for_visualization

        forward_zero_loss = self.mse_loss(out['forward_flow'][0], torch.zeros_like(out['forward_flow'][0]))
        self.loss_data['first_flow'][1] = forward_zero_loss

        if self.video_length > 3:
            flow_curvature = self.get_curvature(seg_registered_list_forward)
        ...
        forward_flow_loss = self.segmentation_loss(seg_registered_list_forward.permute(1, 2, 3, 4, 0), target[1:].permute(1, 2, 3, 4, 0))
        self.loss_data['forward_flow_loss'][1] = forward_flow_loss

        for i in range(1, len(out['forward_flow'])):
            registered = self.motion_estimation(flow=out['forward_flow'][i], original=unlabeled[i - 1])
...
#
    ...
    def run_iteration_train(self, data_generator):
        ...
        rv_strain = data_dict['rv_strain']
        lv_strain = data_dict['lv_strain']
...
#
        out = self.loss_function(unlabeled=unlabeled, out=out, target=target, lv_strain=lv_strain, rv_strain=rv_strain)
    ...
    def run_iteration_overfitting(self, data_generator):
        ...
        if self.log_images:
            ...
            out['seg'] = out['seg'][:self.video_length]
            out['weights'] = out['weights'][:self.video_length]
            out['seg_registered_forward'] = out['seg_registered_forward'][:self.video_length]
            out['registered_input_forward'] = out['registered_input_forward'][:self.video_length]
            out['forward_flow'] = out['forward_flow'][:self.video_length]
    ...
    def warp_linear(self, target, flow):
        ...
        for t in range(1, target.shape[1]):
            ...
            fixed = target[:, t]
            current_flow = flow[:, t]
            moving = torch.nn.functional.one_hot(moving[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()

            #matplotlib.use('QtAgg')
            #fig, ax = plt.subplots(1, 2)
            #ax[0].imshow(torch.argmax(moving[0], dim=0).cpu(), cmap='gray')
            #ax[1].imshow(fixed[0, 0].cpu(), cmap='gray')
            #plt.show()



            current_registered = self.motion_estimation(flow=current_flow, original=moving, mode='bilinear')

            #matplotlib.use('QtAgg')
            #fig, ax = plt.subplots(1, 1)
            #ax.imshow(torch.argmax(current_registered[0], dim=0).cpu(), cmap='gray')
            #plt.show()
            
            registered_list.append(current_registered)
            flow_target_list.append(fixed)

        registered = torch.stack(registered_list, dim=0) # T, B, 4, H, W
        registered = torch.argmax(registered, dim=2, keepdim=True) # T, B, 1, H, W
        flow_target = torch.stack(flow_target_list, dim=0) # T, B, 1, H, W

        flow_target = flow_target[:, :, 0] # T, B, H, W
        registered = registered[:, :, 0] # T, B, H, W
        return registered, flow_target
    

    def get_stats(self, out, target, rv_strain, lv_strain, padding):
        #step = int(len(out['forward_flow']) / len(target))
        #out['forward_flow'] = out['forward_flow'][::step]

        self.compute_strain_mse(lv_strain_pred=out['lv_strain'], 
                                rv_strain_pred=out['rv_strain'], 
                                lv_strain_gt=lv_strain, 
                                rv_strain_gt=rv_strain)
        ...
        registered, flow_target = self.warp_linear(target, flow) # T, B, H, W

        registered = registered.permute(1, 0, 2, 3).contiguous()[:, :, None] # B, T, 1, H, W
        flow_target = flow_target.permute(1, 0, 2, 3).contiguous()[:, :, None] # B, T, 1, H, W

        flow_target = self.processor.uncrop_no_registration(flow_target, padding_need=padding) # B, T, 1, H, W
        registered = self.processor.uncrop_no_registration(registered, padding_need=padding) # B, T, 1, H, W
        flow_target = flow_target[:, :, 0] # B, T, H, W
        registered = registered[:, :, 0] # B, T, H, W
...
#
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                patient_names = np.array([x.split('_')[0] for x in all_keys_sorted])
                to_split = np.arange(len(all_keys_sorted))
                ...
                for i, (train_index, test_index) in enumerate(gss.split(to_split, groups=patient_names)):
                    ...
                    assert len(train_index) + len(test_index) == len(all_keys_sorted)
                    train_keys = np.array(all_keys_sorted)[train_index]
                    test_keys = np.array(all_keys_sorted)[test_index]
    ...
    def run_training_flow(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            with torch.no_grad():
                ...
                self.network.eval()
                val_losses = []
                if self.config['log_stats'] and self.epoch % self.config['overfit_log'] == 0:
    ...
    def get_basic_generators(self):
        ...
        dl_tr = DataLoaderFlowTrainLib(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size, video_length=self.video_length,
        ...
        dl_overfitting = DataLoaderFlowTrainLib(self.dataset_val, self.basic_generator_patch_size, self.patch_size, self.batch_size, video_length=self.video_length,
                                    crop_size=self.crop_size, processor=self.processor, is_val=True, oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
