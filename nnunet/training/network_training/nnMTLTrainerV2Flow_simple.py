...
from nnunet.training.dataloading.dataset_loading import DataLoaderFlowTrainPredictionVal, DataLoaderFlowTrainRandom, DataLoader2D, DataLoader2DMiddleUnlabeled, unpack_dataset, DataLoaderVideoUnlabeled
...
from nnunet.lib.training_utils import build_flow_model_simple, build_2d_model, build_flow_model_4, build_discriminator, read_config_video, read_config, build_discriminator, build_discriminator, build_video_model
...
from nnunet.training.dataloading.dataset_loading import DataLoaderFlowTrainPredictionVal, DataLoaderFlowTrainOneFrame, DataLoaderFlowTrain5, DataLoaderFlowTrain5Lib, DataLoaderFlowValidationOneStep
...
class nnMTLTrainerV2FlowSimple(nnUNetTrainer):
    """
    ...
    def validate_flow_one_step_lib_simple(self, processor, log_function, step, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True,
        ...
        if self.inference_mode == 'one_step':
            if '032' in self.task:
                ret = self.validate_flow_one_step_lib_simple(processor=self.processor, log_function=self.print_to_log_file, step=1, do_mirroring=do_mirroring, use_sliding_window=use_sliding_window, step_size=step_size,
                ...
                ret = super().validate_flow_one_step_simple(processor=self.processor, log_function=self.print_to_log_file, step=1, do_mirroring=do_mirroring, use_sliding_window=use_sliding_window, step_size=step_size,
                                save_softmax=save_softmax, use_gaussian=use_gaussian,
                                overwrite=overwrite, validation_folder_name=validation_folder_name,
                                all_in_gpu=all_in_gpu, segmentation_export_kwargs=segmentation_export_kwargs,
                                run_postprocessing_on_folds=run_postprocessing_on_folds, debug=True, output_folder=output_folder, save_flow=True)
    ...
    def compute_losses_all_to_all(self, target_mask, unlabeled, out, target):
        ...
        current_flow = out['forward_flow'][0]
        initial_target_registered = self.motion_estimation(flow=current_flow, original=initial_target, mode='bilinear')

        seg_registered_loss_target = self.segmentation_loss(initial_target_registered, target[-1])
        self.loss_data['seg_to_seg_forward'][1] = seg_registered_loss_target

        seg_registered_list_forward = torch.argmax(initial_target_registered[None], dim=2)
...
#
        ...
        registered_input_forward = self.motion_estimation(flow=out['forward_flow'][0], original=unlabeled[0])
        ...
        forward_flow_loss = self.mse_loss(out['registered_input_forward'][0], unlabeled[-1])
...
#
    ...
    def get_basic_generators(self):
        ...
        """ dl_tr = DataLoaderFlowTrainRandom(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size, unlabeled_dataset=self.dataset_un_tr, video_length=self.video_length,
                                        crop_size=self.crop_size, processor=self.processor, is_val=False, oversample_foreground_percent=self.oversample_foreground_percent,
                                        pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
        dl_val = DataLoaderFlowTrainPredictionVal(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, unlabeled_dataset=self.dataset_un_val, video_length=self.video_length,
                                    crop_size=self.crop_size, processor=self.processor, is_val=True, oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
        dl_overfitting = DataLoaderFlowTrainRandom(self.dataset_val, self.basic_generator_patch_size, self.patch_size, self.batch_size, unlabeled_dataset=self.dataset_un_val, video_length=self.video_length,
                                    crop_size=self.crop_size, processor=self.processor, is_val=True, oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r') """
