...
from nnunet.lib.training_utils import build_2d_model, build_flow_model_4, build_discriminator, read_config_video, read_config, build_discriminator, build_discriminator, build_video_model
...
class nnMTLTrainerV2Flow5(nnUNetTrainer):
    """
    ...
    def compute_losses(self, target_mask, unlabeled, out, target):
        ...
        current_motion_forward = target[0]
        current_motion_forward = torch.nn.functional.one_hot(current_motion_forward[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
        t_list_forward = []
        t_list_forward_registered = []
        for t in range(len(unlabeled) - 1):
            current_registered = self.motion_estimation(flow=out['forward_flow'][t], original=out['seg'][t], mode='bilinear')
            current_motion_forward = self.motion_estimation(flow=out['forward_flow'][t], original=current_motion_forward, mode='bilinear')
            t_list_forward.append(current_motion_forward)
            t_list_forward_registered.append(current_registered)
        t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
        t_list_forward_registered = torch.stack(t_list_forward_registered, dim=0) # T - 1, B, 4, H, W
        out['seg_registered_forward'] = t_list_forward
        out['pred_registered_forward'] = t_list_forward_registered

        target_pred = torch.softmax(out['seg'], dim=2)
        target_pred = torch.argmax(target_pred, dim=2, keepdim=True) # T, B, 1, H, W

        pred_registered_loss_pred = self.segmentation_loss(out['pred_registered_forward'].permute(1, 2, 3, 4, 0), target_pred[1:].permute(1, 2, 3, 4, 0))
        ...
        seg_registered_loss_target = self.segmentation_loss(out['seg_registered_forward'][t_indices, b_indices], target[1:][t_indices, b_indices])
        self.loss_data['seg_registered_target'][1] = seg_registered_loss_target

        pred_registered_loss_target = self.segmentation_loss(out['pred_registered_forward'][t_indices, b_indices], target[1:][t_indices, b_indices])
        ...
        #    for j in range(i):
        #        cumulated_seg = self.motion_estimation(flow=out['backward_flow'][i - j], original=cumulated_seg)
        #    t_list_backward.append(cumulated_seg)
...
#
    ...
    def compute_losses_ds(self, target_mask, unlabeled, out, target):
        ...
        target_pred = torch.softmax(out['seg'][0], dim=2)
        target_pred = torch.argmax(target_pred, dim=2, keepdim=True) # T, B, 1, H, W

        t_indices, b_indices = torch.nonzero(target_mask[1:], as_tuple=True)
        current_motion_forward = target[0]
        current_motion_forward = torch.nn.functional.one_hot(current_motion_forward[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
        seg_registered_pred_loss_list = []
        seg_registered_target_loss_list = []
        pred_registered_pred_loss_list = []
        pred_registered_target_loss_list = []
        for idx, w in enumerate(self.ds_weights):
            t_list_forward = []
            t_list_forward_registered = []
            for t in range(len(unlabeled) - 1):
                current_registered = self.motion_estimation(flow=out['forward_flow'][idx][t], original=out['seg'][idx][t], mode='bilinear')
                current_motion_forward = self.motion_estimation(flow=out['forward_flow'][idx][t], original=current_motion_forward, mode='bilinear')
                t_list_forward.append(current_motion_forward)
                t_list_forward_registered.append(current_registered)
            t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
            t_list_forward_registered = torch.stack(t_list_forward_registered, dim=0) # T - 1, B, 4, H, W
            ...
            seg_registered_loss_pred = w * self.segmentation_loss(t_list_forward.permute(1, 2, 3, 4, 0), target_pred[1:].permute(1, 2, 3, 4, 0))
            seg_registered_pred_loss_list.append(seg_registered_loss_pred.view(1,))
            
            seg_registered_loss_target = w * self.segmentation_loss(t_list_forward[t_indices, b_indices], target[1:][t_indices, b_indices])
            seg_registered_target_loss_list.append(seg_registered_loss_target.view(1,))

            pred_registered_loss_pred = w * self.segmentation_loss(t_list_forward_registered.permute(1, 2, 3, 4, 0), target_pred[1:].permute(1, 2, 3, 4, 0))
            pred_registered_pred_loss_list.append(pred_registered_loss_pred.view(1,))

            pred_registered_loss_target = w * self.segmentation_loss(t_list_forward_registered[t_indices, b_indices], target[1:][t_indices, b_indices])
            pred_registered_target_loss_list.append(pred_registered_loss_target.view(1,))

        self.loss_data['seg_registered_pred'][1] = torch.cat(seg_registered_pred_loss_list).mean()
        self.loss_data['seg_registered_target'][1] = torch.cat(seg_registered_target_loss_list).mean()
        self.loss_data['pred_registered_pred'][1] = torch.cat(pred_registered_pred_loss_list).mean()
        self.loss_data['pred_registered_target'][1] = torch.cat(pred_registered_target_loss_list).mean()
...
#
        ...
        image_loss_list = []
        flow_regularization_loss_list = []
        for idx, w in enumerate(self.ds_weights):
            t_list_forward = []
            current_motion_forward = unlabeled[0]
            for t in range(len(unlabeled) - 1):
                current_motion_forward = self.motion_estimation(flow=out['forward_flow'][idx][t], original=current_motion_forward)
                t_list_forward.append(current_motion_forward)
            t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
            if idx == 0:
                out['registered_input_forward'] = t_list_forward
            forward_flow_loss = w * self.mse_loss(t_list_forward, unlabeled[1:])
            image_loss_list.append(forward_flow_loss.view(1,))
            flow_regularization_loss_forward = w * self.image_flow_loss(flow=out['forward_flow'][idx][:-1], iter_nb=self.iter_nb)
            flow_regularization_loss_list.append(flow_regularization_loss_forward.view(1,))
        
        self.loss_data['image_flow'][1] = torch.cat(image_loss_list).mean()
        self.loss_data['flow_regularization'][1] = torch.cat(flow_regularization_loss_list).mean()
...
#
    ...
    def compute_losses_all_to_all(self, target_mask, unlabeled, out, target):
        ...
        initial_target_registered_list = torch.stack(initial_target_registered_list, dim=0) # T - 1, B, 4, H, W
        ...
        pred_registered_loss_target = self.segmentation_loss(pred_registered_list[t_indices, b_indices], target_all[t_indices, b_indices])
        self.loss_data['pred_registered_target'][1] = pred_registered_loss_target
...
#
    ...
    def compute_losses_one_to_all(self, target_mask, unlabeled, out, target):
        ...
        for t in range(1, len(unlabeled)):
            initial_target_registered = self.motion_estimation(flow=out['forward_flow'][t], original=initial_target, mode='bilinear')
            t_list_forward.append(initial_target_registered)
        t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
        out['seg_registered_forward'] = t_list_forward
...
#
    ...
    def compute_losses_one_to_all_ds(self, target_mask, unlabeled, out, target):
        out['registered_input_backward'] = []
        out['registered_input_forward'] = []
        out['seg_registered_forward'] = []
        ...
        for idx, w in enumerate(self.ds_weights):
            ...
            t_list_forward = []
            for t in range(1, len(unlabeled)):
                current_seg_registered = self.motion_estimation(flow=out['forward_flow'][idx][t], original=current_motion_forward, mode='bilinear')
                t_list_forward.append(current_seg_registered)
            t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
            if idx == 0:
                out['seg_registered_forward'] = t_list_forward
            seg_registered_loss_pred = w * self.segmentation_loss(t_list_forward.permute(1, 2, 3, 4, 0), target_pred[1:].permute(1, 2, 3, 4, 0))
...
#
        ...
        for idx, w in enumerate(self.ds_weights):
            t_list_forward = []
            for t in range(1, len(unlabeled)):
                registered_input_forward = self.motion_estimation(flow=out['forward_flow'][idx][t], original=unlabeled[0])
                t_list_forward.append(registered_input_forward)
            t_list_forward = torch.stack(t_list_forward, dim=0) # T - 1, B, 4, H, W
            if idx == 0:
                out['registered_input_forward'] = t_list_forward
            forward_flow_loss = w * self.mse_loss(t_list_forward, unlabeled[1:])
...
#
    ...
    def run_iteration_val(self, data_generator):
        ...
        out = self.network(unlabeled)
        out = self.loss_function(target_mask=target_mask, unlabeled=unlabeled, out=out, target=target)
...
#
    ...
    def get_basic_generators(self):
        ...
        #                            pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
        dl_val = DataLoaderFlowTrain5(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, unlabeled_dataset=self.dataset_un_val, video_length=self.video_length,
