...
from nnunet.lib.training_utils import build_2d_model, build_flow_model, build_confidence_network, read_config, build_discriminator, build_discriminator, build_video_model
...
from nnunet.training.dataloading.dataset_loading import DataLoaderFlow
...
class nnMTLTrainerV2Flow(nnUNetTrainer):
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.fine_tuning = True if self.video_length > 1 else False
    ...
    def setup_loss_functions(self, loss_weights):
        ...
        self.image_flow_loss = nn.MSELoss()
    ...
    def initialize_network(self):
        ...
        self.network = build_flow_model(self.config, conv_layer=conv_layer, norm=norm_2d, image_size=self.crop_size)
        if self.fine_tuning:
            self.load_video_weights(self.network)
        labeled_input_data = torch.randn(self.config['batch_size'], 1, self.crop_size, self.crop_size)
        unlabeled_input_data = torch.randn(self.video_length, self.config['batch_size'], 1, self.crop_size, self.crop_size)
        models['temporal_model'] = (self.network, [labeled_input_data, unlabeled_input_data])
    ...
    def get_gradient_images(self, labeled, unlabeled, indices):
        ...
        with torch.enable_grad():
            ...
            output = self.network(labeled, unlabeled)
            predictions = output['unlabeled_predictions'].softmax(dim=2)
            out = predictions[t, b, c, y, x]
            out.backward()
            gradient_image = torch.abs(unlabeled.grad)
            gradient_image = gradient_image[:, b, 0, :, :]
            unlabeled = unlabeled[:, b, 0]
            return gradient_image, unlabeled
    ...
    def start_online_evaluation(self, out,
        ...
        uncrop_target = self.processor.uncrop_no_registration(target, padding_need=padding_need)
        assert list(uncrop_target.shape[-2:]) == [self.image_size, self.image_size]
        pred = out['unlabeled_predictions'][0]
        ...
        with torch.no_grad():
            ...
            output_seg = self.processor.uncrop_no_registration(output_seg, padding_need=padding_need)
            ...
            if self.log_images:
                for b in range(len(target)):
                    ...
                    motion_flow = flow_to_image(out['motion_flow_u_to_l'][:, b]) # T, 3, H, W
                    ...
                    self.vis.set_up_image_flow(seg_dice=seg_dice[b].mean(), 
                                                moving=unlabeled[:, b, 0], 
                                                registered_input=out['registered_input_u_to_l'][:, b, 0],
                                                fixed=labeled[b, 0],
                                                motion_flow=motion_flow,
                                                registered_seg=registered_seg[:, b],
    ...
    def finish_online_evaluation(self):
        ...
        if self.log_images:
            ...
            self.vis.log_motion_images(colormap=cmap, norm=norm, epoch=self.epoch)
    ...
    def run_online_evaluation(self, out, labeled, unlabeled, target, padding_need):
        """
        ...
        indices = self.sample_indices(target[None])
        gradient_image, gradient_x = self.get_gradient_images(labeled, unlabeled, indices)
    ...
    def compute_losses(self, labeled, unlabeled, out, target):
        assert len(unlabeled) == len(out['unlabeled_predictions'])
        ...
        for i in range(len(unlabeled)):
            seg_loss_l = self.segmentation_loss(out['labeled_predictions'][i], target)
            self.loss_data['segmentation'][1] = seg_loss_l

            image_flow_loss_u_to_l = self.image_flow_loss(out['registered_input_u_to_l'][i], labeled)
            image_flow_loss_l_to_u = self.image_flow_loss(out['registered_input_l_to_u'][i], unlabeled[i])
            self.loss_data['image_flow'][1] = (image_flow_loss_u_to_l + image_flow_loss_l_to_u) / 2

            seg_loss_flow = self.segmentation_loss(out['registered_seg'][i], target)
    ...
    def get_heatmap_gt(self, target):
        target[target > 0] = 1
        target = target.squeeze(1) # N, crop_size, crop_size
        heatmaps = torch.zeros_like(target).float()
        
        print(target.shape)
        reduced_target = torch.flatten(target, start_dim=1)
        mask = torch.count_nonzero(reduced_target, dim=-1) > 0
        non_null_idx = torch.where(mask)[0]
        print(mask)
        print(non_null_idx)
        ...
        if torch.any(mask):
            coords = masks_to_boxes(filtered_target)
            x = coords[:, 0] + ((coords[:, 2] - coords[:, 0]) / 2)
            y = coords[:, 1] + ((coords[:, 3] - coords[:, 1]) / 2)
            centroids = torch.stack([x, y], dim=-1) # N, 2
            centroids = torch.round(centroids).long() # N, 2
            heatmaps[non_null_idx, centroids[:, 0], centroids[:, 1]] = 1.
            heatmaps = gaussian_blur(heatmaps, kernel_size=15)

        matplotlib.use('QtAgg')
        fig, ax = plt.subplots(2, 2)
        ax[0, 0].imshow(heatmaps[0].cpu(), cmap='plasma')
        ax[0, 1].imshow(target[0].cpu(), cmap='gray')
        ax[1, 0].imshow(heatmaps[1].cpu(), cmap='plasma')
        ax[1, 1].imshow(target[1].cpu(), cmap='gray')
        plt.show()
    ...
    def run_iteration_train(self, data_generator):
        ...
        out = self.network(labeled, unlabeled)
        self.compute_losses(labeled=labeled, unlabeled=unlabeled, out=out, target=target)
...
#
    ...
    def get_basic_generators(self):
        ...
        dl_un_tr = None
        dl_un_val = None
        dl_tr = DataLoaderFlow(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size, is_val=False, unlabeled_dataset=self.dataset_un_tr, video_length=self.video_length, step=self.step, force_one_label=self.force_one_label,
        ...
        dl_val = DataLoaderFlow(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, is_val=True, unlabeled_dataset=self.dataset_un_val, video_length=self.video_length, step=self.step, force_one_label=self.force_one_label,
                                    crop_size=self.crop_size, processor=self.processor, oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
