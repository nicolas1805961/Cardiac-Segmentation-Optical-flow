...
from diffusers import DDPMScheduler, DDPMPipeline, UNet2DModel, AutoencoderKL, ControlNetModel, UNet2DConditionModel, StableDiffusionPipeline
...
class nnMTLTrainerV2ControlNet(nnUNetTrainer):
    """
    ...
    def initialize_network(self):
        ...
        self.vae = AutoencoderKL.from_pretrained("vae_folder").to('cuda:0')
        self.tokenizer = CLIPTokenizer.from_pretrained("tokenizer_folder")
        self.text_encoder = CLIPTextModel.from_pretrained("text_encoder_folder").to('cuda:0')
        self.network = self.get_model().to('cuda:0')
        unlabeled_input_data = torch.randn(self.config['batch_size'], 4, 16, 16)
        time_input_data = torch.randn(self.config['batch_size'],)
        condition_data = torch.randn(self.config['batch_size'], self.tokenizer.model_max_length, self.text_encoder.config.hidden_size)
        models['temporal_model'] = (self.network, [unlabeled_input_data, time_input_data, condition_data])
...
#
    ...
    def run_iteration_train(self, data_generator):
        ...
        with torch.no_grad():
            ...
            encoder_hidden_states = self.text_encoder(uncond_input.input_ids.to('cuda:0'))[0]
            latents = self.vae.encode(clean_images).latent_dist.sample()
            latents = latents * self.vae.config.scaling_factor

        noise = torch.randn(latents.shape).to(latents.device)
        bs = latents.shape[0]
        timesteps = torch.randint(0, self.diffusion_scheduler.config.num_train_timesteps, (bs,), device=latents.device).long()
        #t = torch.randint(0, self.diffusion_scheduler.timesteps, (self.batch_size,)).long().to('cuda:0')
        noisy_images = self.diffusion_scheduler.add_noise(latents, noise, timesteps)
        #noisy_images = self.q_sample(latents, t, noise=noise)
        noise_pred = self.network(noisy_images, timesteps, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
    ...
    def run_iteration_val(self):
        ...
        if self.log_images:
            pipeline = StableDiffusionPipeline(vae=self.vae, 
            ...
            images = pipeline(prompt=[""]*self.batch_size, generator=torch.manual_seed(0), num_inference_steps=999, guidance_scale=0.0, output_type='np').images
...
#
            ...
            normalized_list = []
            for i in range(len(images)):
                image = cv.normalize(images[i], None, alpha=0, beta=255, norm_type=cv.NORM_MINMAX).astype(np.uint8)
                normalized_list.append(image)
            images = np.stack(normalized_list, axis=0)
