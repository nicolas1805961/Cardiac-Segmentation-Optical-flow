...
from ..lib.vit_transformer import TransformerLib, BeforeFlow, TransformerFlowEncoderAllOnlyContext, TransformerFlowEncoderFirst, TransformerFlowEncoderAllSeparate
...
class OpticalFlowModelLib(SegmentationNetwork):
    def __init__(self,
                deep_supervision,
                out_encoder_dims,
                in_dims,
                ...
                all_to_all,
                only_first):
        super(OpticalFlowModelLib, self).__init__()
        ...
        self.flow_decoder = decoder_alt.Decoder2D(dot_multiplier=dot_multiplier, deep_supervision=deep_supervision, conv_depth=conv_depth_decoder, in_encoder_dims=decoder_in_dims[::-1], out_encoder_dims=out_encoder_dims[::-1], num_classes=2, img_size=image_size, last_activation='tanh')
        ...
        self.bottleneck = TransformerLib(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers, video_length=video_length)
        #self.before_flow = BeforeFlow(dim=self.d_model, nhead=self.bottleneck_heads, nb_layers=self.nb_layers)
    
        self.skip_co_reduction_list_flow = nn.ModuleList()
        for idx, dim in enumerate(out_encoder_dims):
            reduction_flow = nn.Conv2d(in_channels=dim * 2, out_channels=dim, kernel_size=1)
            self.skip_co_reduction_list_flow.append(reduction_flow)
    ...
    def forward(self, unlabeled):
        ...
        for t in range(len(forward)):
            ...
            for s, reduction_layer in enumerate(self.skip_co_reduction_list_flow):
                previous_index = t - 1 if t > 0 else 0
                concatenated_forward = torch.cat([unlabeled_skip_co_list[previous_index][s], unlabeled_skip_co_list[t][s]], dim=1)
            ...
            out['forward_flow'].append(current_global_motion_forward)
        ...
        out['lv_strain'] = lv_strain
        out['rv_strain'] = rv_strain
        out['weights'] = weights
    ...
    def _internal_maybe_mirror_and_pred_2D_2(self, unlabeled, target, processor, mirror_axes: tuple,
        ...
        for i in tqdm(range(1, len(cropped_unlabeled))):
            current_cropped_unlabeled = cropped_unlabeled[:i+1]

            result_torch_seg = torch.zeros([current_cropped_unlabeled.shape[0], unlabeled.shape[1], self.num_classes, processor.crop_size, processor.crop_size], dtype=torch.float)
            ...
            for m in range(mirror_idx):
                ...
                if m == 1 and (1 in mirror_axes):
                    current_cropped_unlabeled_flipped = torch.flip(current_cropped_unlabeled, (4, ))
                    output = self(current_cropped_unlabeled_flipped)
                    seg_pred = torch.softmax(output['seg'], dim=2)
                ...
                if m == 2 and (0 in mirror_axes):
                    current_cropped_unlabeled_flipped = torch.flip(current_cropped_unlabeled, (3, ))
                    output = self(current_cropped_unlabeled_flipped)
                    seg_pred = torch.softmax(output['seg'], dim=2)
                ...
                if m == 3 and (0 in mirror_axes) and (1 in mirror_axes):
                    current_cropped_unlabeled_flipped = torch.flip(current_cropped_unlabeled, (4, 3))
                    output = self(current_cropped_unlabeled_flipped)
                    seg_pred = torch.softmax(output['seg'], dim=2)
            ...
            if i == 1:
                seg_list.append(result_torch_seg)
            else:
                seg_list.append(result_torch_seg[-1][None])
            global_motion_list.append(result_torch_global_motion)

        result_torch_seg = torch.cat(seg_list, dim=0)
    ...
    def _internal_maybe_mirror_and_pred_2D(self, unlabeled, target, processor, mirror_axes: tuple,
        ...
        result_torch_lv_strain = torch.zeros(list(unlabeled.shape[:2]), dtype=torch.float)
        result_torch_rv_strain = torch.zeros(list(unlabeled.shape[:2]), dtype=torch.float)
        if torch.cuda.is_available():
            unlabeled = to_cuda(unlabeled, gpu_id=self.get_device())
            #target = to_cuda(target, gpu_id=self.get_device())
            result_torch_seg = result_torch_seg.cuda(self.get_device(), non_blocking=True)
        ...
        for m in range(mirror_idx):
            if m == 0:
                ...
                lv_strain_pred = output['lv_strain']
                rv_strain_pred = output['rv_strain']
                ...
                result_torch_lv_strain = lv_strain_pred
                result_torch_rv_strain = rv_strain_pred
                #result_torch_flow += 1 / num_results * flow_pred
        ...
        return result_torch_seg, result_torch_flow, result_torch_lv_strain, result_torch_rv_strain, cropped_target, padding_need
    ...
    def _internal_predict_3D_2Dconv_tiled_flow(self, unlabeled, target, target_mask, processor, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for depth_idx in range(unlabeled.shape[2]):
            ...
            pred_seg, softmax_pred, flow_pred, registered_pred, lv_strain, rv_strain = self._internal_predict_2D_2Dconv_tiled_flow(
            ...
            lv_strain_list.append(lv_strain[None])
            rv_strain_list.append(rv_strain[None])
        ...
        lv_strain_pred = np.vstack(lv_strain_list).transpose((1, 0)) # T, depth
        rv_strain_pred = np.vstack(rv_strain_list).transpose((1, 0)) # T, depth

        return predicted_segmentation, softmax_pred, flow_pred, registered_pred, lv_strain_pred, rv_strain_pred
...
#
        ...
        seg, flow, lv_strain, rv_strain, target, padding_need = self._internal_maybe_mirror_and_pred_2D(unlabeled_data_input, target_input, processor, mirror_axes, do_mirroring)
        ...
        lv_strain = lv_strain[:, 0]
        rv_strain = rv_strain[:, 0]

        if all_in_gpu:
            flow = flow.half()
            seg = seg.half()
            all_motions = all_motions.half()
            lv_strain = lv_strain.half()
            rv_strain = rv_strain.half()
        else:
            flow = flow.cpu().numpy()
            seg = seg.cpu().numpy()
            all_motions = all_motions.cpu().numpy()
            lv_strain = lv_strain.cpu().numpy()
            rv_strain = rv_strain.cpu().numpy()
...
#
        ...
        return predicted_segmentation, class_probabilities, aggregated_results_flow, aggregated_results_registered, lv_strain, rv_strain
    ...
    def warp_linear(self, flow, target):
        registered_list = []
        first = to_cuda(target[0], gpu_id=self.get_device())
        registered_list.append(first)
        for t in range(1, len(target)):
            current_motion = target[t - 1]
            current_motion = torch.nn.functional.one_hot(current_motion[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
            current_motion = to_cuda(current_motion, gpu_id=self.get_device())
            current_flow = flow[t]
