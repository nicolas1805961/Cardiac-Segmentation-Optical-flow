...
from ..lib.encoder import Encoder, Encoder1D, Encoder2D
...
from ..lib.vit_transformer import TransformerFlowSegEncoderMutual, TransformerFlowSegEncoder2, TransformerFlowSegEncoderLabel, TransformerFlowEncoderFirst, TransformerFlowEncoderAllSeparate
...
class OpticalFlowModelLabeled(SegmentationNetwork):
    def __init__(self,
                deep_supervision,
                out_encoder_dims,
                in_dims,
                ...
                only_first,
                norm_2d):
        super(OpticalFlowModelLabeled, self).__init__()
        ...
        in_dims_label_encoder = in_dims[:]
        in_dims_label_encoder[0] = 5
        self.label_encoder = Encoder2D(out_dims=out_encoder_dims, in_dims=in_dims_label_encoder, conv_depth=conv_depth)

        decoder_in_dims = in_dims[:]
        decoder_in_dims[0] = self.num_classes
        conv_depth_decoder = conv_depth[::-1]

        self.seg_decoder = decoder_alt.Decoder2D(dot_multiplier=dot_multiplier, deep_supervision=deep_supervision, conv_depth=conv_depth_decoder, conv_layer=conv_layer_2d, dpr=dpr_decoder, in_encoder_dims=decoder_in_dims[::-1], out_encoder_dims=out_encoder_dims[::-1], num_classes=self.num_classes, img_size=image_size, norm=norm_2d)
        self.flow_decoder = decoder_alt.Decoder2D(dot_multiplier=dot_multiplier, deep_supervision=deep_supervision, conv_depth=conv_depth_decoder, conv_layer=conv_layer_2d, dpr=dpr_decoder, in_encoder_dims=decoder_in_dims[::-1], out_encoder_dims=out_encoder_dims[::-1], num_classes=2, img_size=image_size, norm=norm_2d)
        ...
        if self.only_first:
            ...
            self.flow_seg_encoder = TransformerFlowSegEncoderMutual(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers)

        self.skip_co_reduction_list = nn.ModuleList()
        self.label_distillation_list = nn.ModuleList()
        for idx, dim in enumerate(out_encoder_dims):
            reduction = nn.Conv2d(in_channels=dim * 2, out_channels=dim, kernel_size=1)
            label_distillation = nn.Conv2d(in_channels=dim * 2, out_channels=dim, kernel_size=1)
            self.skip_co_reduction_list.append(reduction)
            self.label_distillation_list.append(label_distillation)
    ...
    def forward(self, label, unlabeled):
        out = {'forward_flow': [],
               'backward_flow': []}
        ...
        for t in range(len(unlabeled)):
            if t == 0:
                label = torch.nn.functional.one_hot(label[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()
                label = torch.cat([unlabeled[0], label], dim=1)
            else:
                label = torch.nn.functional.pad(unlabeled[t], pad=(0, 0, 0, 0, 0, 4))
            
            #matplotlib.use('QtAgg')
            #fig, ax = plt.subplots(1, label.shape[1])
            #for u in range(label.shape[1]):
            #    ax[u].imshow(label[0, u].cpu(), cmap='gray')
            #plt.show()
            
            unlabeled_features, unlabeled_skip_connections = self.label_encoder(label)
        ...
        for t in range(len(frames)):
            ...
            seg_skip_co = []
            for s, reduction_layer in enumerate(self.label_distillation_list):
                ...
                seg_skip_co.append(concatenated_forward)

            frames_seg = self.seg_decoder(frames[t], seg_skip_co)
            frames_seg_list.append(frames_seg)

            if t == 0:
                anchor_seg = self.seg_decoder(anchor.mean(0), seg_skip_co)
        ...
        #previous = torch.cat([out['forward_flow'][0][None], out['forward_flow'][:-1]])
        #following = torch.cat([out['forward_flow'][1:], out['forward_flow'][-1][None]])
        #out['forward_flow'] = (previous + following) / 2
    ...
    def _internal_maybe_mirror_and_pred_2D(self, unlabeled, target, processor, mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
                output = self(cropped_target[0], cropped_unlabeled)
            ...
            if m == 1 and (1 in mirror_axes):
                ...
                cropped_target_flipped = torch.flip(cropped_target, (4, ))
                output = self(cropped_target_flipped[0], cropped_unlabeled_flipped)
                seg_pred = torch.softmax(output['seg'], dim=2)
                flow_pred = output['forward_flow']
            ...
            if m == 2 and (0 in mirror_axes):
                ...
                cropped_target_flipped = torch.flip(cropped_target, (3, ))
                output = self(cropped_target_flipped[0], cropped_unlabeled_flipped)
                seg_pred = torch.softmax(output['seg'], dim=2)
                flow_pred = output['forward_flow']
            ...
            if m == 3 and (0 in mirror_axes) and (1 in mirror_axes):
                ...
                cropped_target_flipped = torch.flip(cropped_target, (4, 3))
                output = self(cropped_target_flipped[0], cropped_unlabeled_flipped)
                seg_pred = torch.softmax(output['seg'], dim=2)
                flow_pred = output['forward_flow']
...
#
        ...
        return predicted_segmentation, class_probabilities, aggregated_results_flow, aggregated_results_registered
    ...
    def warp_linear_sequential(self, flow, indices, target):
        ...
        for t in range(indices[0].item(), indices[1].item()):
            ...
            registered_list.append(torch.argmax(current_motion, dim=1, keepdim=True))
    ...
    def warp_linear_all_to_all(self, flow, target):
        ...
        for t in range(len(target)):
            if t == 0:
                ...
                incremental_flow = flow[t] - flow[0]
