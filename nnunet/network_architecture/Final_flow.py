...
from nnunet.lib.utils import MotionEstimation, MLP, DeformableTransformer, ConvBlocks2DGroupLegacy, ConvBlock, ConvBlocks3DEmbedding, ConvBlocks2DGroup, ConvBlocks2DEmbedding, GetSeparability, GetCrossSimilarityMatrix, ReplicateChannels, To_image, From_image, rescale, CCA
...
from nnunet.lib.vit_transformer import SpatioTemporalTransformer, SpatioTemporalTransformerSeparated, TransformerContext, TransformerFlowEncoderSuccessiveNoEmb, TransformerFlowEncoderFromStartNoEmb, TransformerFlowEncoderFromStart, TransformerFlowEncoderAllDistance, TransformerFlowSegEncoderAggregationDistance, TransformerFlowSegEncoderAggregationDistanceNoEmb, TransformerFlowEncoderAllOnlyContext, TransformerFlowEncoderFirst, TransformerFlowEncoderAllSeparate
...
class FinalFlow(SegmentationNetwork):
    def __init__(self,
                deep_supervision,
                out_encoder_dims,
                in_dims,
        ...
        in_dims_encoder[0] = nb_channels
        in_dims_past[0] = 6

        self.encoder_current = Encoder2D(out_dims=out_encoder_dims, in_dims=in_dims_encoder, conv_depth=conv_depth, norm=norm, legacy=legacy, nb_conv=nb_conv)
        self.encoder_past = Encoder2D(out_dims=out_encoder_dims, in_dims=in_dims_past, conv_depth=conv_depth, norm=norm, legacy=legacy, nb_conv=nb_conv)
        ...
        if self.bottleneck_type == 'gru':
            self.gru_cell = ConvGRUCell(input_size=(int(image_size / 2**(self.num_stages)), int(image_size / 2**(self.num_stages))),
                                    input_dim=self.d_model,
                                    hidden_dim=self.d_model,
                                    kernel_size=(3, 3),
                                    bias=True,
                                    dtype=torch.cuda.FloatTensor)
            self.bottleneck = TransformerContext(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers)
        elif self.bottleneck_type == '3d':
            self.conv_3d = nn.Conv3d(in_channels=self.d_model, out_channels=self.d_model, kernel_size=3, padding='same')
        elif self.bottleneck_type == 'transformer':
            self.bottleneck = SpatioTemporalTransformer(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers, topk=self.topk, pos_1d=pos_1d)
        elif self.bottleneck_type == 'transformer_bis':
                self.bottleneck = SpatioTemporalTransformerSeparated(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers, topk=self.topk, pos_1d=pos_1d, distance=distance)

        #if gru:
        #    self.gru_cell_list = nn.ModuleList()
        #    for idx, dim in enumerate(out_encoder_dims):
        #        gru_cell = ConvGRUCell(input_size=(int(image_size / 2**idx), int(image_size / 2**idx)),
        #                                    input_dim=dim,
        #                                    hidden_dim=dim,
        #                                    kernel_size=(3, 3),
        #                                    bias=True,
        #                                    dtype=torch.cuda.FloatTensor)
        #        self.gru_cell_list.append(gru_cell)
        #    
        #    self.gru_dim = out_encoder_dims
    ...
    def forward(self, x, step=1):
        if self.bottleneck_type == 'gru':
            return self.forward_memory_gru(x, step=step)
        elif self.bottleneck_type == '3d':
            return self.forward_memory_3d(x, step=step)
        elif self.bottleneck_type == 'transformer':
            return self.forward_memory(x, step=step)
        elif self.bottleneck_type == 'transformer_bis':
            return self.forward_memory_separated(x, step=step)


    def forward_memory(self, x, step=1):
        out = {'flow': []}
        T, B, C, H, W = x.shape
        cumulated = torch.zeros(size=(B, 2, H, W), device=x.device)

        #current_feature, skip_co = self.encoder_current(x[0]) # B, C, H, W
        #current_feature = self.bottleneck(current_feature, current_feature[None])
        #acc_skip_co = []
        #for s in range(self.num_stages):
        #    concatenated = torch.cat([skip_co[s], skip_co[s]], dim=1)
        #    concatenated = self.skip_co_reduction_list[s](concatenated)
        #    acc_skip_co.append(concatenated)
        #
        #flow = self.flow_decoder(current_feature, acc_skip_co)
        #seg = self.segmentation_decoder(current_feature, skip_co)
        #out['seg'].append(seg)
        ...
        past_skip_co_list = []
        for t in range(1, T):
            registered = self.motion_estimation(flow=cumulated, original=x[t - 1])
            error = x[0] - registered

            #matplotlib.use('QtAgg')
            #fig, ax = plt.subplots(1, 1)
            #ax.imshow(error[0, 0].cpu(), cmap='plasma')
            #ax.axis('off')
            #plt.show()
            #plt.close(fig)
            #current_frame_img = cv.normalize(error[0, 0].cpu().numpy(), None, alpha=0, beta=255, norm_type=cv.NORM_MINMAX).astype(np.uint8)
            #imC = cv.applyColorMap(current_frame_img, cv.COLORMAP_HOT)
            #cv.imwrite(f'error_{t}.png', imC)

            past_input = torch.cat([x[t - 1], x[0], cumulated, registered, error], dim=1)
            ...
            if not self.training:
                past_features = torch.cat([past_features[0][None], past_features[step:-1:step], past_features[-1][None]])

            current_feature = self.bottleneck(current_feature, past_features)
    ...
    def forward_memory_separated(self, x, step=1):
        out = {'flow': []}
        T, B, C, H, W = x.shape
        cumulated = torch.zeros(size=(B, 2, H, W), device=x.device)
        ...
        for t in range(1, T):
            ...
            if not self.training:
                past_features = torch.cat([past_features[0][None], past_features[step:-1:step], past_features[-1][None]])
                past_query = torch.cat([past_query[0][None], past_query[step:-1:step], past_query[-1][None]])

            current_feature = self.bottleneck(query=current_feature, key=past_query, value=past_features)
...
#
    #        past_query_list.append(current_feature)
#
    #        current_input = torch.cat([x[t], x[t - 1], x[0]], dim=1)
    #        current_feature, skip_co_current = self.encoder_current(current_input) # B, C, H, W
#
    #        past_features = torch.stack(past_feature_list, dim=0) # T, B, C, H, W
    #        past_query = torch.stack(past_query_list, dim=0) # T, B, C, H, W
...
#
    ...
    def forward_memory_gru(self, x, step=1):
        out = {'flow': []}
        T, B, C, H, W = x.shape
        cumulated = torch.zeros(size=(B, 2, H, W), device=x.device)
        ...
        h = torch.zeros(size=(B, self.d_model, int(self.image_size / 2**self.num_stages), int(self.image_size / 2**self.num_stages)), device=x.device)
        ...
        for t in range(1, T):
            ...
            h = self.gru_cell(input_tensor=past_features[-1], h_cur=h) # (b,t,c,h,w)
            current_feature = self.bottleneck(current_feature, h)
...
#
    ...
    #    #out['flow'].append(flow)
#
    #    h_list = [torch.zeros(size=(B, dim, int(self.image_size / 2**idx), int(self.image_size / 2**idx)), device=x.device) for idx, dim in enumerate(self.gru_dim)]
#
    #    past_feature_list = []
    #    past_skip_co_list = []
    #    for t in range(1, T):
...
#
    #        acc_skip_co = []
    #        for s in range(self.num_stages):
    #            h_list[s] = self.gru_cell_list[s](input_tensor=past_skip_co_list[-1][s], h_cur=h_list[s]) # (b,t,c,h,w)
    #            concatenated = torch.cat([skip_co_current[s], past_skip_co_list[-1][s], h_list[s]], dim=1)
    #            concatenated = self.skip_co_reduction_list[s](concatenated)
    #            acc_skip_co.append(concatenated)
#
    #        flow = self.flow_decoder(current_feature, acc_skip_co)
...
#
    ...
    def forward_embedding(self, x):
        ...
        T, B, C, H, W = x.shape
        cumulated = torch.zeros(size=(B, 2, H, W), device=x.device)
        cumulated_embedding = torch.zeros(size=(B, self.d_model, int(H / 8), int(W / 8)), device=x.device)
        ...
        for t in range(1, T):
            ...
            current_feature = self.bottleneck_1(current_feature, past_new)
            embedding = self.bottleneck_2(cumulated_embedding, current_feature)
            current_feature = self.bottleneck_3(current_feature, embedding)

            acc_skip_co = []
            for s in range(self.num_stages):
                concatenated = torch.cat([skip_co_current[s], skip_co_past_new[s]], dim=1)
                concatenated = self.skip_co_reduction_list[s](concatenated)
                acc_skip_co.append(concatenated)

            flow = self.flow_decoder(current_feature, acc_skip_co)
            cumulated = cumulated + flow[0]
            cumulated_embedding = cumulated_embedding + embedding
            out['flow'].append(cumulated)
