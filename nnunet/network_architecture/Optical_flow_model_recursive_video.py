...
from ..lib.vit_transformer import TransformerFlowSegEncoderAggregation, BeforeFlow, TransformerFlowEncoderAllOnlyContext, TransformerFlowEncoderFirst, TransformerFlowEncoderAllSeparate
...
class OpticalFlowModelRecursiveVideo(SegmentationNetwork):
    def __init__(self,
                deep_supervision,
                out_encoder_dims,
                in_dims,
                ...
                all_to_all,
                only_first):
        super(OpticalFlowModelRecursiveVideo, self).__init__()
        ...
        self.bottleneck = TransformerFlowSegEncoderAggregation(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers)
    ...
    def forward(self, unlabeled):
        ...
        forward, global_motion_forward = self.bottleneck(unlabeled_features)

        #segmentations
        #middle = torch.stack([backward[1:], forward[:-1]], dim=0)
        #segs = torch.cat([backward[0][None], middle.mean(0), forward[-1][None]])

        #middle = torch.stack([global_motion_backward[1:], global_motion_forward[:-1]], dim=0)
        #segs = torch.cat([global_motion_backward[0][None], middle.mean(0), global_motion_forward[-1][None]])

        for t in range(len(global_motion_forward)):
            current_seg = self.seg_decoder(global_motion_forward[t], unlabeled_skip_co_list[t])
        ...
        for t in range(len(forward)):
            ...
            local_motion_forward = self.flow_decoder(forward[t], flow_skip_co_forward)
            out['local_motions_forward'].append(local_motion_forward)
        
        out['local_motions_forward'] = self.organize_deep_supervision(out['local_motions_forward'])
        ...
        for t in range(len(forward)):
            ...
            current_global_motion_forward = self.flow_decoder(global_motion_forward[t], flow_skip_co_forward)
            out['global_motion_forward'].append(current_global_motion_forward)
        ...
        for i in tqdm(range(1, len(cropped_unlabeled))):
            ...
            for m in range(mirror_idx):
                if m == 0:
                    ...
                    global_motion = output['global_motion_forward'][-1]
