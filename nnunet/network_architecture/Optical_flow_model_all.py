...
from ..lib.vit_transformer import _get_clones, TransformerDecoder, TransformerFlowEncoderAll, TransformerFlowEncoder, ModulationTransformer, TransformerFlowDecoder, SpatioTemporalTransformer, SpatialTransformerLayer, SlotTransformer, TransformerEncoderLayer, SlotAttention, CrossTransformerEncoderLayer, CrossRelativeSpatialTransformerLayer
...
class OpticalFlowModel(SegmentationNetwork):
    def __init__(self,
        ...
        self.seg_decoder_all = decoder_alt.SegmentationDecoder2(dot_multiplier=dot_multiplier, deep_supervision=deep_supervision, conv_depth=conv_depth_decoder, conv_layer=conv_layer, dpr=dpr_decoder, in_encoder_dims=in_dims[::-1], out_encoder_dims=out_encoder_dims[::-1], num_classes=self.num_classes, img_size=image_size, norm=norm_2d)
        self.flow_decoder_all = decoder_alt.SegmentationDecoder2(dot_multiplier=dot_multiplier, deep_supervision=deep_supervision, conv_depth=conv_depth_decoder, conv_layer=conv_layer, dpr=dpr_decoder, in_encoder_dims=in_dims[::-1], out_encoder_dims=out_encoder_dims[::-1], num_classes=2 * (video_length - 1), img_size=image_size, norm=norm_2d)

        self.mid_conv_list = nn.ModuleList()
        for idx, dim in enumerate(out_encoder_dims):
            conv = conv_layer(in_dim=dim, out_dim=dim/video_length, nb_blocks=conv_depth[idx],  kernel_size=3, dpr=[0], norm=norm_2d)
            self.mid_conv_list.append(conv)

        H, W = (int(image_size / 2**(self.num_stages)), int(image_size / 2**(self.num_stages)))
        #d_ffn = min(2048, self.d_model * 4)
        
        self.transformer_flow_encoder = TransformerFlowEncoderAll(dim=self.d_model, nhead=self.bottleneck_heads, num_layers=self.nb_layers, video_length=video_length, nb_tokens=nb_tokens)
    ...
    def forward(self, unlabeled):
        ...
        unlabeled_features = self.transformer_flow_encoder(spatial_features=unlabeled_features) # T, B, H*W, C ; T, B, M, C

        all_flow_list = []
        for s, reduction_layer in enumerate(self.mid_conv_list):
            flow_skip_co = []
            for t in range(len(unlabeled_skip_co_list)):
                concatenated = reduction_layer(unlabeled_skip_co_list[t][s])
                flow_skip_co.append(concatenated)
            flow_skip_co = torch.cat(flow_skip_co, dim=1)
            all_flow_list.append(flow_skip_co)
        
        to_decode_feature = unlabeled_features.permute(0, 2, 1).contiguous().view(B, C, H, W)

        flow = self.flow_decoder_all(to_decode_feature, all_flow_list)
        flow = self.flow_decoder_all(to_decode_feature, all_flow_list)
        seg_1_list = []
        for t in range(len(unlabeled_skip_co_list)):
            seg_1 = self.seg_decoder_all(to_decode_feature, unlabeled_skip_co_list[t])
            seg_1_list.append(seg_1)

        flow = torch.split(flow, 2, dim=1)
        seg = torch.stack(seg_1_list, dim=0)

        #middle = torch.stack([seg_1[1:], seg_2[:-1]], dim=0).mean(0)
        #seg = torch.cat([seg_1[0][None], middle, seg_2[-1][None]], dim=0)

        out['seg'] = seg
        out['forward_flow'] = torch.stack(flow, dim=0)
        out['backward_flow'] = torch.stack(flow, dim=0)
