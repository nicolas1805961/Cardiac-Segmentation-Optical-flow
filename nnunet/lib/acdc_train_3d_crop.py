import torch
from acdc_dataset import create_3d_acdc_dataset_crop
...
from training_utils import build_policy_net, build_discriminator, set_losses, set_augmentations, read_config, build_3d_model_crop, create_loggers, count_parameters
...
img_size = 160
train_path = 'ACDC_resampled/*'
val_path = 'ACDC_resampled/*'
loss_weights = torch.tensor(config['160_resampled_loss_weights'], device=config['device'])
window_size = [12, 10, 10]
...
dataloaders = create_3d_acdc_dataset_crop(train_path=train_path, 
                                    val_path=val_path,
...
if config['reinforcement']:
    ...
    model_input_size = (config['batch_size'], 1, 12, img_size, img_size)
