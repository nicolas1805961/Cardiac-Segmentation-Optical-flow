import torch
from acdc_dataset import create_acdc_dataset_logits_3d
...
from training_utils import build_policy_net, build_discriminator, set_losses, set_augmentations, read_config, build_3d_model_logits, create_loggers, count_parameters
...
img_size = 128
train_path = 'ACDC_logits_2/*'
val_path = 'ACDC_logits_2/*'
loss_weights = torch.tensor(config['128_resampled_loss_weights'], device=config['device'])
window_size = [9, 8, 8]
...
dataloaders = create_acdc_dataset_logits_3d(train_path=train_path, 
                                    val_path=val_path,
...
if config['reinforcement']:
    ...
    model = build_3d_model_logits(config)
    ...
    model_input_size = (config['batch_size'], 1, 9, img_size, img_size)
