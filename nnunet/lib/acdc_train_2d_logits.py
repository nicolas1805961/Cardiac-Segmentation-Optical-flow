import torch
from acdc_dataset import create_acdc_dataset_logits_2d
...
from training_utils import get_img_size_and_dataset_name, build_policy_net, build_discriminator, set_losses, set_augmentations, read_config, build_model_logits, create_loggers, count_parameters
...
config = read_config('adversarial_acdc.yaml')

model2=None
optimizer2 = None
scheduler2 = None

img_size, train_path, val_path, loss_weights, window_size = get_img_size_and_dataset_name(config, logits=True)
...
dataloaders = create_acdc_dataset_logits_2d(train_path=train_path, 
                                        val_path=val_path,
...
if config['reinforcement']:
    ...
    model = build_model_logits(config)
    ...
    model_input_size = [(config['batch_size'], 4, img_size, img_size), (config['batch_size'], 4, img_size, img_size)]
...
try:
    if config['reinforcement']:
        ...
        loop.main_loop_acdc_supervised_2d()
