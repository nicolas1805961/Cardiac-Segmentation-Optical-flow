...
class MultiheadCrossAttention(Module):
    ...
    def __init__(self, 
                ...
                q_length,
                k_length,
                q_relative_position_index,
                k_relative_position_index,
                proj,
                device,
                num_memory_token=0,
                rpe_mode='contextual',
        ...
        self.proj = proj
        self.device=device
        self.q_relative_position_index = q_relative_position_index
        self.k_relative_position_index = k_relative_position_index
        ...
        q_rpe_table_size = (2 * q_length - 1)
        k_rpe_table_size = (2 * k_length - 1)
        if rpe_mode == 'contextual':
            if 'q' in rpe_contextual_tensor:
                self.q_rpe_table = nn.Parameter(torch.zeros(num_heads, embed_dim//num_heads, q_rpe_table_size))  # nh, head_dim, 2*Wh-1 * 2*Ww-1
            if 'k' in rpe_contextual_tensor:
                self.k_rpe_table = nn.Parameter(torch.zeros(num_heads, embed_dim//num_heads, k_rpe_table_size))  # nh, head_dim, 2*Wh-1 * 2*Ww-1
            if 'v' in rpe_contextual_tensor:
                self.v_rpe_table = nn.Parameter(torch.zeros(num_heads, k_rpe_table_size, embed_dim//num_heads))  # nh, head_dim, 2*Wh-1 * 2*Ww-1
        elif rpe_mode == 'bias':
            self.rpe_table = nn.Parameter(torch.zeros(num_heads, q_rpe_table_size))  # nh, 2*Wh-1 * 2*Ww-1
    ...
    def __setstate__(self, state):
        ...
        super(MultiheadCrossAttention, self).__setstate__(state)
    ...
    Shapes for outputs:
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = multi_head_attention_forward(
                ...
                q_rpe_indices=self.q_relative_position_index,
                k_rpe_indices=self.k_relative_position_index,
...
def multi_head_attention_forward(
    ...
    #
    attn_output, attn_output_weights = _scaled_dot_product_attention(q, k, v, bsz, num_heads, device, rpe_weight_table, rpe_weight_table_q, rpe_weight_table_k, rpe_weight_table_v, q_rpe_indices, k_rpe_indices, attn_mask, dropout_p)
...
def _scaled_dot_product_attention(
    ...
    if rpe_weight_table_v is not None and rpe_weight_table is None:
        w = rpe_weight_table_v[:, k_rpe_indices.flatten()] # 24 49*49 32
