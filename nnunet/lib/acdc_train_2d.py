import torch
from acdc_dataset import create_2d_acdc_dataset, create_2d_acdc_dataset_crop
...
from training_utils import build_policy_net, build_discriminator, set_losses, set_augmentations, read_config, build_2d_model, create_loggers, write_model_parameters, count_parameters, get_validation_images_lib, log_metrics
import argparse
...
if args.train_path == 'ACDC_cropped_multiclass':
    loss_weights = torch.tensor(config['128_transformed_loss_weights'], device=config['device'])
elif args.train_path == 'ACDC_cropped_binary':
    loss_weights = torch.tensor(config['128_loss_weights'], device=config['device'])
elif args.train_path == 'ACDC_resampled_125' or args.train_path == 'ACDC_resampled_125_scipy':
    loss_weights = torch.tensor(config['224_loss_weights'], device=config['device'])
elif args.train_path == 'ACDC_cropped_binary_125':
    loss_weights = torch.tensor(config['128_loss_weights_125'], device=config['device'])

img_size = config['image_size']

if img_size == 352:
    loss_weights = torch.tensor(config['352_loss_weights'], device=config['device'])
...
dataloaders = create_2d_acdc_dataset(train_path=train_path, 
                                    ...
                                    writer=writer,
                                    sampler_param=config['sampler_param'],
                                    use_cropped_images=config['use_cropped_images'],
                                    learn_transforms=config['learn_transforms'],
...
if config['reinforcement']:
    ...
    policy_net_optimizer = torch.optim.AdamW(policy_net.parameters(), lr=config['policy_net_learning_rate'], weight_decay=config['weight_decay'])
    ...
    model_optimizer = torch.optim.AdamW(model.parameters(), lr=config['learning_rate'], weight_decay=config['weight_decay'])
...
if config['deep_supervision']:
    nb_weights = len(config['in_encoder_dims'])
    if config['directional_field']:
        nb_weights += 1
    deep_supervision_weights = [1 / 2**x for x in reversed(range(0, nb_weights))]
    deep_supervision_weights = torch.tensor(deep_supervision_weights)
    deep_supervision_weights = (deep_supervision_weights / deep_supervision_weights.sum()).tolist()
else:
    if config['directional_field']:
        deep_supervision_weights = [0.1, 0.9]
    else:
...
loop = reconstruction_loop.ReconstructionLoop(labeled_train_dataloader=dataloaders['labeled_train_dataloader'],
                                                ...
                                                mode=config['model'],
                                                directional_field=config['directional_field'],
                                                directional_field_weight=config['directional_field_weight'],
                                                uncertainty_weighting=config['uncertainty_weighting'],
                                                dynamic_weight_averaging=config['dynamic_weight_averaging'],
                                                ...
                                                overfitting_dataloader=dataloaders['overfitting_dataloader'],
