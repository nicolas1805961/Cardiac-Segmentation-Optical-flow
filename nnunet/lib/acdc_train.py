...
from acdc_dataset import create_acdc_dataset_alt
...
loss_weights = torch.tensor(config['loss_weights_binary'], device=config['device'])
data_augmentation_utils = set_augmentations(config, data_augmentation, autoencoder=False)
data_augmentation_utils = None if not data_augmentation_utils else data_augmentation_utils
dataloaders = create_acdc_dataset_alt(path=config['path'] + '/*', 
...
count_parameters(model, console_logger, file_logger, config, 'U-net', autoencoder=None, discriminator=None)
