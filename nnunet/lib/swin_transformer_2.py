...
from .utils import get_indices_2d, depthwise_separable_conv, Mlp
...
class WindowAttentionConvRpe(nn.Module):
    ...
    def __init__(self, norm, dim, attention_map, window_size, proj, num_heads, rpe_mode, rpe_contextual_tensor, input_resolution, device, relative_position_index=None, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if attention_map:
            stride = int(128 / input_resolution[-1])
            self.attention_conv = nn.Conv2d(in_channels=1, out_channels=num_heads, kernel_size=1, stride=stride)

        if proj == 'conv':
            self.qkv = depthwise_separable_conv(dim, 3*dim, norm=norm)
        elif proj == 'linear':
    ...
    def forward(self, x, mask=None, attention_map=None):
        """
        Args:
        ...
        if attention_map is not None:
            assert attention_map.dim() == 4
            batch_size = attention_map.shape[0]
            attention_map = self.attention_conv(attention_map).permute(0, 2, 3, 1)
            attention_map = window_partition(attention_map, self.window_size)  # nW * B, window_size, window_size, num_heads
            attention_map = attention_map.view(-1, self.window_size * self.window_size)
            attention_map = attention_map.unsqueeze(1) - attention_map.unsqueeze(2) # B*nw*num_heads, window_size*window_size, window_size*window_size

            nW = attention_map.shape[0] / (self.num_heads * batch_size)
            attention_map = attention_map.view(batch_size, nW, self.num_heads, N, N)
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) * attention_map
            attn = attn.view(-1, self.num_heads, N, N)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, norm, dim, attention_map, input_resolution, proj, use_conv_mlp, num_heads, device, relative_position_index, rpe_mode, rpe_contextual_tensor, window_size, shift_size,
    ...
    def forward(self, x, attention_map=None):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask, attention_map=attention_map)  # nW*B, window_size*window_size, C
...
class BasicLayerUp(nn.Module):
    ...
    def __init__(self, dim, norm, attention_map, input_resolution, shortcut, proj, use_conv_mlp, depth, num_heads, device, rpe_mode, rpe_contextual_tensor, window_size,
    ...
    def forward(self, x, attention_map=None):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x, attention_map=attention_map)
        if self.shortcut:
            x = x + shortcut
            x = self.out_norm(x)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, norm, dim, proj, attention_map, shortcut, input_resolution, use_conv_mlp, depth, num_heads, device, rpe_mode, rpe_contextual_tensor, window_size=7,
