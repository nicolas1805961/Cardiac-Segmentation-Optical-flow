...
def from_ed(patient_name, phase_list_pkl, phase_list_img, phase_list):
    ...
    gt_path = os.path.join(phase_list_pkl[0].split(os.sep)[0], patient_name, patient_name + '_frame' + str(ed_number).zfill(2) +'_gt.nii.gz')
    gt_data = nib.load(gt_path)
    gt = gt_data.get_fdata().astype(int)
    gt[gt > 0] = 1
    ...
    for d in range(len(flow)):
        current_gt = gt[:, :, d]
        regions = regionprops(current_gt)
        ...
        current_gt = current_gt[bbox[0]-20:bbox[2]+20, bbox[1]-20:bbox[3]+20]

        video_name = filename[:-4] + '.mp4'
        print(video_name)
        W, H = current_gt.shape[0], current_gt.shape[1]
        ...
        out_list = []
        for t in range(1, len(current_slice_flow)):
            current_contours = contours[t].numpy() # P, 2
            ...
            current_frame_img = cv.cvtColor(current_frame_img, cv.COLOR_GRAY2RGB)


            for p in range(len(current_contours)):
                first_contours_numpy = first_contours.squeeze()
                first_contours_numpy = first_contours_numpy.permute(1, 0).numpy()
                gt_points_start = tuple([x for x in np.round(first_contours_numpy[p]).astype(int)])
                gt_points_end = tuple([x for x in np.round(current_contours[p]).astype(int)])
                pred_points_end = tuple([x for x in np.round(new_predicted_points[p]).astype(int)])
                current_frame_img = cv.circle(current_frame_img, gt_points_end, radius=1, color=(0, 255, 0), thickness=-1)
                current_frame_img = cv.circle(current_frame_img, pred_points_end, radius=1, color=(255, 0, 0), thickness=-1)
                current_frame_img = cv.line(current_frame_img, gt_points_start, gt_points_end, (0, 255, 0), thickness=1)
                current_frame_img = cv.line(current_frame_img, gt_points_start, pred_points_end, (255, 0, 0), thickness=1)
            

            #fig, ax = plt.subplots(1, 1)
            #ax.imshow(current_frame_img)
            #plt.show()

            current_frame_img = current_frame_img[bbox[0]-20:bbox[2]+20, bbox[1]-20:bbox[3]+20]
            current_frame_img = cv.cvtColor(current_frame_img, cv.COLOR_RGB2BGR)
            video.write(current_frame_img)

            #out_list.append(current_frame_img)
        cv.destroyAllWindows()
        video.release()

        #out_list = np.stack(out_list, axis=0)

        #slice_error_list.append(out_list)
    #out_list = np.stack(slice_error_list, axis=0) # D, T, 3

    #np.savez(patient_name + '_video.npz', video=out_list)
...
if __name__ == "__main__":
    ...
    registered_patients = ['patient269']
