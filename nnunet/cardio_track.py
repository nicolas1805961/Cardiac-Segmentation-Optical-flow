...
import numpy as np
from code_nicolas_short_axis_rv import *
...
def main():
    ...
    assert len(patients) == 1, 'The input folder must contain only one .gz file'
    ...
    path_list = glob(os.path.join(output_folder, patients[0].split(os.sep)[-1], 'Registered', '*.gz'))

    contour_list_rv = []
    contour_list_lv_epi = []
    contour_list_lv_endo = []
    ...
    for i in range(len(path_list)):
        ...
        mask_lv_endo,cnt_lv_endo = get_contour(arr, 3)
        mask_lv_epi, cnt_lv_epi = get_contour(arr, [2, 3])
        mask_rv, cnt_rv = get_contour(arr, 1)

        cnt_septum = extract_adj_contour2(cnt_lv_epi, arr,[1,2],precision=4)
        ...
        refpoints_list_rv.append(refpoints_rv)
        refpoints_list_lv.append(refpoints_lv)
        ...
        point_jolie_rv = np.append(point_jolie_rv,[point_jolie_rv[0]],axis=0)
        point_jolie_lv_endo = np.append(point_jolie_lv_endo,[point_jolie_lv_endo[0]],axis=0)
        point_jolie_lv_epi = np.append(point_jolie_lv_epi,[point_jolie_lv_epi[0]],axis=0)

        contour_list_rv.append(point_jolie_rv)
        contour_list_lv_endo.append(point_jolie_lv_endo)
        contour_list_lv_epi.append(point_jolie_lv_epi)

    peak_index = 0
    resX = 1.5

    contour_list_rv_inter = []
    contour_list_lv_endo_inter = []
    contour_list_lv_epi_inter = []

    for struct_contour in [(contour_list_rv, contour_list_rv_inter), 
                          (contour_list_lv_endo, contour_list_lv_endo_inter),
                          (contour_list_lv_epi, contour_list_lv_epi_inter)]:
        X = struct_contour[0][peak_index][:, 0]
        Y = struct_contour[0][peak_index][:, 1]
        nb_point = len(interpolate_points(X, Y, resX))
        for i in range(len(struct_contour[0])):
            X = struct_contour[0][i][:, 0]
            Y = struct_contour[0][i][:, 1]
            interpolated = interpolate_points(X, Y, resX, nb_point)
            struct_contour[1].append(interpolated)


    structure_AI = {'RV':{
                        'refPoint':refpoints_list_rv.tolist(),
                        'contour_IA':{
                            'X1' : to_mat_struct(contour_list_rv_inter,0),
                            'Y1' : to_mat_struct(contour_list_rv_inter,1)}
                        },
                    'LV':{
                        'refPoint':refpoints_list_lv.tolist(),
                        'contour_IA':{
                            'X1' : to_mat_struct(new_patient.contour.LA,0),
                            'Y1' : to_mat_struct(new_patient.contour.LA,1)}
                        }
                    }
